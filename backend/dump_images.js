...
async function checkImages() {
    try {
        ...
        const { rows: products } = await pool.query("SELECT id, name, image FROM products");
        products.forEach(p => console.log(`ID ${p.id}: ${p.name} -> ${p.image}`));

        console.log('\n--- Product Images Table ---');
        const { rows: images } = await pool.query("SELECT product_id, image_url FROM product_images");
